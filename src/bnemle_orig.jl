...
function bnemle(data::Union{String,DataFrame}, query::String; digits::Int64=4, k::Number=1, outfile::String="", bootstraps::Int64=0, mincounts::Array=[1,20], showids=false, outfile2::String="", rrrdenom::Array=[])
    ...
    function bnemlebootstrap(dfg, bootstraps, rvals) # Input is original df
        ...
        if bootstraps > 0
            ...
            if 1 < bootstraps < 1000
                printstyled("WARN::Use at least 1000 bootstraps for better estimates.\n", color=:yellow)
            end
            ...
            TF = mean(B1);      TFlc = B1[Int64(ceil(length(B1) * 0.025)) ];     TFuc = B1[Int64(floor(length(B1) * 0.975))];
            ARR = mean(B2);     ARRlc = B2[Int64(ceil(length(B2) * 0.025)) ];    ARRuc = B2[Int64(floor(length(B2) * 0.975))];
            lp_ARR = mean(B3);  lp_ARRlc = B3[Int64(ceil(length(B3) * 0.025)) ]; lp_ARRuc = B3[Int64(floor(length(B3) * 0.975))];
            RRR = mean(B4);     RRRlc = B4[Int64(ceil(length(B4) * 0.025)) ];    RRRuc = B4[Int64(floor(length(B4) * 0.975))];
            lp_RRR = mean(B5);  lp_RRRlc = B5[Int64(ceil(length(B5) * 0.025)) ]; lp_RRRuc = B5[Int64(floor(length(B5) * 0.975))];
            ...
            if cc == bootstraps + 1
                ...
                println("ARR             $ARR($ARRlc, $ARRuc)") #ARR (adj)       $lp_ARR($lp_ARRlc, $lp_ARRuc)")
                println("RRR             $RRR($RRRlc, $RRRuc)") #RRR (adj)       $lp_RRR($lp_RRRlc, $lp_RRRuc)")
                println("-"^70)
