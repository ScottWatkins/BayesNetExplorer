"""
    ...
    Inputs: a csv file or dataframe and the probability query. The input data must have sample labels in column 1, and all columns must be labeled. Input will be converted to string values.

Currently, only binary conditional variables are allowed. Laplace corrected risk ratios are provided. Outfiles can be made for bootstrap runs, and a significant flag is provided based on significance and mincounts. 
...
"""
function bnemle(data::Union{String,DataFrame}, query::String; digits::Int64=4, k::Number=1, outfile::String="", bootstraps::Int64=0, mincounts::Array=[1,20], showids=false, outfile2::String="", rrrdenom::Array=[], confmeth="t-dist")
    ...
    function runbnemle(df, newcols, z)
        ...
        function laplace_correction(k, states, all_c, cond_c, cond_c_opp, targ_c, t_feat_c, t_feat_c_fopp)
            ...
            if t_feat_c == 0 || t_feat_c_fopp == 0  # lp correction of num and denom if
                ...
                t_c_adj = t_feat_c                  # but don't correct if no zero states
                a_c_adj = cond_c        
