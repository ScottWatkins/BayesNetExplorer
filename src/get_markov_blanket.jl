...
Return the markov blanket of a target node for a DAG. Features is an array with the column names of the variables in the DAG. The target is the integer or name that specifies the target's col id in the DAG. The markov blanket contains the target's parents and offspring and the parents of the offspring. Input DAG format, D[i,j] must be D[parent,offspring], that is, arrow goes from row to column.
"""
